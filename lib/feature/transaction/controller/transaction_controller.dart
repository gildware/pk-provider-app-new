...
class TransactionController extends GetxController implements GetxService{
  ...
  Future<void> getWithdrawMethods({bool isReload = false}) async{
    if(_withdrawModel == null || isReload) {
      ...
      if(response.statusCode == 200 && response.body['response_code'] == 'default_200' && response.body['content'] != null) {
        ...
        for (final element in _withdrawModel?.withdrawalMethods ?? <WithdrawalMethod>[]) {
          if (!element.isActiveForSelection) continue;
          if (element.isDefault == 1) {
            defaultWithdrawMethod = element;
            break;
          }
          defaultWithdrawMethod ??= element;
        }
        if (defaultWithdrawMethod != null) {
          _defaultPaymentMethodId = defaultWithdrawMethod.id;
          _defaultPaymentMethodName = defaultWithdrawMethod.methodName;
        }
      }else{
  ...
  /// Loads saved payment methods and withdrawal method types for the withdraw screen.
  Future<void> loadWithdrawMethodOptions({bool isReload = false}) async {
    await Get.find<PaymentInfoController>().getPaymentMethods(isUpdate: false, isReload: isReload);
    await getWithdrawMethods(isReload: isReload);
  ...
  void _processWithdrawalMethods() {
    for (final WithdrawalMethod method in _withdrawModel?.withdrawalMethods ?? []) {
      if (!method.isActiveForSelection) continue;

...
class ServiceCategoryController extends GetxController implements GetxService{
  ...
  Future<void> getSubCategoryList({required int offset, bool isFromPagination = false, String? categoryId}) async {
    ...
    if(response.statusCode == 200){
      ...
      for (var subCategory in list) {
        final model = ServiceSubCategoryModel.fromJson(subCategory);
        if ((model.servicesCount ?? 0) > 0) {
          serviceSubCategoryList.add(model);
        }
      }
  ...
  Future<ResponseModel> changeSubscriptionStatus(String id, int index,{String fromPage = ""}) async {
      final confirmed = await Get.find<UserProfileController>().confirmProfileReviewSubmission();
      if (!confirmed) {
      ...
      if(response.statusCode==200){
        ...
        if (content is Map && content['submitted_for_review'] == true) {
        ...
        if(fromPage == "category"){
          ...
          Get.find<SubcategorySubscriptionController>().refreshSubscribedCategoryIds();
        }
  ...
  void changeCategoryToSubscribedMenuIndex(int subscriptionMenuIndex) {
    if (subscriptionMenuIndex <= 0 || serviceCategoryList == null) {
      return;
    }
    final subscribedCategoryIds = Get.find<SubcategorySubscriptionController>().subscribedCategoryIds;
    final subscribedCategories = serviceCategoryList!
        .where((category) => category.id != null && subscribedCategoryIds.contains(category.id))
        .toList();
    if (subscriptionMenuIndex - 1 >= subscribedCategories.length) {
      return;
    }
    final category = subscribedCategories[subscriptionMenuIndex - 1];
    final fullIndex = serviceCategoryList!.indexWhere((c) => c.id == category.id);
    if (fullIndex >= 0) {
      changeCategory(fullIndex);
    }
  }

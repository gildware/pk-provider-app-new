  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GetBuilder<SubcategorySubscriptionController>(
        initState: (_){
          final subscriptionController = Get.find<SubcategorySubscriptionController>();
          subscriptionController.getMySubscriptionData(1, false);
          subscriptionController.refreshSubscribedCategoryIds();
        },
          ...
          child: Column(children: [
            ...
            GetBuilder<ServiceCategoryController>(
                builder: (serviceCategoryController) {
                  final subscribedCategoryIds = mySubscriptionController.subscribedCategoryIds;
                  final categoriesWithSubscriptions = (serviceCategoryController.serviceCategoryList ?? [])
                      .where((category) => category.id != null && subscribedCategoryIds.contains(category.id))
                      .toList();

                  if (serviceCategoryController.selectedSubsCategoryIndex > categoriesWithSubscriptions.length) {
                    WidgetsBinding.instance.addPostFrameCallback((_) {
                      serviceCategoryController.changeSubscriptionCategoryIndex(0);
                      mySubscriptionController.getMySubscriptionData(1, false);
                    });
                  }

                  List<ServiceCategoryModel> serviceCategoryList =[];
                  serviceCategoryList.add(ServiceCategoryModel(name: 'all'.tr));
                  serviceCategoryList.addAll(categoriesWithSubscriptions);
                  return Container(
                    color:  Theme.of(context).cardColor,
                    width: double.infinity,
                    height: 50,

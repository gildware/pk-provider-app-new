...
bool _isUsableImageUrl(String url) {
  if (url.isEmpty) return false;
  final lower = url.toLowerCase();
  if (lower.contains('placeholder') && !lower.contains('/provider/')) {
    return false;
  }
...
class Content {
  ...
  Content.fromJson(Map<String, dynamic> json) {
    providerInfo = json['provider_info'] != null
        ...
        : null;
    hasPendingProfileChanges = json['has_pending_profile_changes'] == true
        || json['has_pending_profile_changes'] == 1;
    hasPendingBrandingChanges = json['has_pending_branding_changes'] == true
        || json['has_pending_branding_changes'] == 1;
    final preview = json['pending_branding_preview'];
    if (preview is Map) {
      final logo = preview['logo_url']?.toString().trim() ?? '';
      final cover = preview['cover_url']?.toString().trim() ?? '';
      pendingBrandingLogoUrl = logo.isNotEmpty ? logo : null;
      pendingBrandingCoverUrl = cover.isNotEmpty ? cover : null;
    }
...
class ProviderInfo {
  ...
  List<String>? zoneIds;
  String? providerType;
  String? contactPersonPhotoFullPath;
  String? companyIdentityType;
  String? companyIdentityNumber;
  List<String>? companyIdentityImagesFullPath;
  Owner? owner;
  ...
  bool get isCompanyProvider => providerType == 'company';

  String? get displayLogoUrl {
    final fromApi = logoFullPath?.trim();
    if (fromApi != null && fromApi.isNotEmpty) {
      return MobileAppIconHelper.resolveMediaUrl(fromApi) ?? fromApi;
    }
    final file = logo?.trim();
    if (file != null && file.isNotEmpty) {
      return MobileAppIconHelper.resolveMediaUrl('/storage/provider/logo/$file');
    }
    return null;
  }
  ...
  ProviderInfo(
      {this.id,
        ...
        this.zoneIds,
        this.providerType,
        this.contactPersonPhotoFullPath,
        this.companyIdentityType,
        this.companyIdentityNumber,
        this.companyIdentityImagesFullPath,
        this.owner,
        this.isSuspend,
  ...
  ProviderInfo.fromJson(Map<String, dynamic> json) {
    ...
    final logoPath = json['logo_full_path']?.toString().trim() ?? '';
    logoFullPath = _isUsableImageUrl(logoPath) ? logoPath : null;
    final coverPath = json['cover_image_full_path']?.toString().trim() ?? '';
    coverFullPath = _isUsableImageUrl(coverPath) ? coverPath : null;
    ...
    if (json['zone_ids'] != null) {
      zoneIds = (json['zone_ids'] as List).map((e) => e.toString()).toList();
    }
    providerType = json['provider_type']?.toString();
    final photoPath = json['contact_person_photo_full_path']?.toString().trim() ?? '';
    contactPersonPhotoFullPath = _isUsableImageUrl(photoPath) ? photoPath : null;
    companyIdentityType = json['company_identity_type']?.toString();
    companyIdentityNumber = json['company_identity_number']?.toString();
    if (json['company_identity_images_full_path'] != null) {
      companyIdentityImagesFullPath = (json['company_identity_images_full_path'] as List)
          .map((e) => e?.toString().trim() ?? '')
          .where((e) => e.isNotEmpty && _isUsableImageUrl(e))
          .toList();
    }
...
class Owner {
  ...
  Owner.fromJson(Map<String, dynamic> json) {
    ...
    if (json['identification_image_full_path'] != null) {
      identificationImageFullPath = (json['identification_image_full_path'] as List)
          .map((e) => e?.toString().trim() ?? '')
          .where((e) => e.isNotEmpty && _isUsableImageUrl(e))
          .toList();
    } else {

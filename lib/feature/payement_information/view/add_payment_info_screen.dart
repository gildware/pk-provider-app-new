...
class _AddPaymentInfoScreenState extends State<AddPaymentInfoScreen> {
  ...
  Future<void> loadData() async {
    ...
    if (!isEdit) {
      _applyDefaultMethod(transactionController);
    } else {
      final matchedMethod = transactionController.withdrawModel?.withdrawalMethods?.firstWhere(
      ...
      if (_selectedMethodId.isNotEmpty) {
  ...
  void _applyDefaultMethod(TransactionController transactionController) {
    final methods = _selectableWithdrawMethods(transactionController);
    ...
    _selectedMethodName = defaultMethod.methodName ?? '';
    if (_selectedMethodId.isNotEmpty) {
      selectPaymentMethodField(_selectedMethodId, _selectedMethodName, transactionController);
    }
  }

  List<WithdrawalMethod> _selectableWithdrawMethods(TransactionController transactionController) {
    return (transactionController.withdrawModel?.withdrawalMethods ?? [])
        .where((m) => m.isActiveForSelection)
        .toList();
  }
                        ...
                        Expanded(
                          child: Container(
                            ...
                            child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              ...
                              if (_methodsLoading)
                              ...
                              else if (_selectableWithdrawMethods(transactionMoneyController).isEmpty)
                                Container(
                                  ...
                                  child: Text(
                                    'no_withdrawal_methods_available'.tr,
                                    style: robotoRegular.copyWith(
                                      fontSize: Dimensions.fontSizeSmall,
                                ...
                                ..._selectableWithdrawMethods(transactionMoneyController).map((withdraw) {
                                  final isSelected = _selectedMethodId == withdraw.id;
                                  return Padding(
                            ...
                            Expanded(
                              child: CustomButton(
                                ...
                                onPressed: () async {
                                  ...
                                  final selectable = _selectableWithdrawMethods(transactionMoneyController);
                                  if (selectable.isEmpty) {
                                    showCustomSnackBar('no_withdrawal_methods_available'.tr, type: ToasterMessageType.info);
                                    return;
                                  }

                                  WithdrawalMethod withdrawMethod = selectable.firstWhere(

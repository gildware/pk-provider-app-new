...
class BusinessSettingController extends GetxController implements GetxService{
  ...
  // SERVICEMAN_DISABLED — restore keys when AppFeatureFlags.servicemanEnabled
  List<String> settings = AppFeatureFlags.servicemanEnabled
      ? [
  ...
  Future<void> updateBookingSettingsIntoServer() async {
    ...
    if(response.statusCode == 200){
      ...
      final submitted = content is Map && content['submitted_for_review'] == true;
      showCustomSnackBar(
        submitted ? 'profile_changes_submitted_for_review'.tr : 'successfully_updated'.tr,
